...
public class EsSearchComponent {
    ...
    private void updateDoc(VideoInfo videoInfo){
        try {
            ...
            throw new BusinessException("保持视频失败");
        }
    }
    ...
    public PaginationResultVO<VideoInfo> search(Boolean highlight,String keword,Integer orderType,Integer pageNo,Integer pageSize){
        try {
            SearchOrderTypeEnum searchOrderTypeEnum= SearchOrderTypeEnum.getByType(orderType);
            SearchSourceBuilder searchSourceBuilder= new SearchSourceBuilder();
            searchSourceBuilder.query(QueryBuilders.multiMatchQuery(keword,"videoName","tags"));
            //高亮
            if(highlight){
